...
flowchart TD
    ...
    H1 --- S1
    H2 --- S1
    S1 ---|Port 3 / 10 Mbps| S2
    S1 ---|Port 4 / 10 Mbps| S3
...
sequenceDiagram
    ...
    Ryu->>S1: OFPT_FLOW_MOD (Priority 40: src=10.0.0.12:80 -> SET src=10.0.0.100, OUT:h1)
    Ryu->>S2: OFPT_FLOW_MOD (Priority 50 and 40: Bidirectional Transit Forwarding)
    Ryu->>S4: OFPT_FLOW_MOD (Priority 50: OUT:srv2, Priority 40: OUT:s2)
    ...
    S4->>S2: Forwarded Pkt (Matched Priority 40 Egress Rule)
    S2->>S1: Transit Forward
    Note over S1: Matched Priority 40 Flow Rule<br/>SET ipv4_src=10.0.0.100<br/>SET eth_src=00:00:00:00:00:fe
    ...
    S1->>S2: Forwarded (Rewritten to backend IP)
    S2->>S4: Forwarded
    S4->>Server: Received HTTP GET
    Server->>S4: HTTP 200 Response
    S4->>S2: Forwarded
    S2->>S1: Forwarded
    S1->>Client: Rewritten to VIP -> Received HTTP 200
```
...
1. **Active Probing (`HealthChecker`):**
   ...
   - If an instance fails consecutive checks (configured limit: 5 failures, interval: 10s), the controller removes the node from active load balancing scheduling.
